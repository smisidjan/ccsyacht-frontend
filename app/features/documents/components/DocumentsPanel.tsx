...
function resolveDocumentStatus(doc: Document) {
  if (doc.approvalStatus != null) return doc.approvalStatus;
  ...
  if (!totalRequired && acks.length === 0) return "pending_review" as const;
  return calculateDocumentStatus(acks, totalRequired);
}
      ...
      <div className="p-4 lg:p-6 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between gap-3 mb-3">
          ...
          <div className="flex items-center gap-2">
            {canEditDocumentTypes && !isReadOnly && !selectedType?.isSystemManaged && (
        ...
        {selectedType && canEditDocumentTypes && !selectedType.isSystemManaged && (
          <AssigneesList
        ...
        {documents && documents.length > 0 && !selectedType?.isSystemManaged && (
...
function DeclinedNote({ doc }: { doc: Document }) {
  const reason = doc.reviewers?.find((r) => r.declineReason)?.declineReason;
      ...
      columns={[
        {
          ...
          cell: (doc: Document) => (
            <div className="flex flex-col gap-1 min-w-0">
              ...
              {resolveDocumentStatus(doc) === "declined" && <DeclinedNote doc={doc} />}
            </div>
          ),
        },
        {
          ...
          header: t("size"),
          cell: (doc: Document) => (
            <span className="text-sm text-gray-600 dark:text-gray-400">{doc.contentSize}</span>
          ),
        },
        {
          key: "uploadedBy",
          header: t("uploadedBy"),
          cell: (doc: Document) => (
            <span className="text-sm text-gray-600 dark:text-gray-400">{doc.author.name}</span>
          ),
        },
        {
          key: "uploadedAt",
          ...
          : [
              {
                key: "status",
                header: t("status"),
                cell: (doc: Document) => <DocumentStatusBadge status={resolveDocumentStatus(doc)} />,
              },
            ]),
        {
          key: "actions",
